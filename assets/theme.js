      ...
      value: function _updateItemQuantity(event, target) {
        ...
        fetch("".concat(window.routes.cartChangeUrl, ".js"), {
          ...
          cart.json().then(function (content) {
            ...
            _this2._rerenderCart(elementToAnimate).then(function () {
...
var pendingAddonRequests = {};
var addonPromotionSyncPromise = null;
var GIFT_BOX_VARIANT = 54540457804055;
var FREE_GIFT_VARIANT = 54684641886487;
var GIFT_BOX_THRESHOLD = 200000;
// Removes lines created by the previous client-side sleepwear workaround.
// Shopify discounts, rather than JavaScript, now set the ₹0 line price.
var LEGACY_SLEEPWEAR_FREE_MARKER = '_storytailor_sleepwear_free';
var FREE_GIFT_TIERS = [
    { threshold: GIFT_BOX_THRESHOLD, quantity: 1 }
];
...
function getCartSubtotalExcludingGiftBoxes(cart) {
  var items = cart.items || [];

  return items.reduce(function (total, item) {
    if (item.variant_id == FREE_GIFT_VARIANT || item.variant_id == GIFT_BOX_VARIANT) {
      return total;
    }
...
function initAddonPromotion() {
  if (addonPromotionSyncPromise) {
    return addonPromotionSyncPromise;
  }

  addonPromotionSyncPromise = fetch('/cart.js')
    .then(function (response) {
      return response.json();
    })
    .then(function (cart) {
      return removeLegacySleepwearFreeLines(cart).then(function (legacyChanged) {
        return (legacyChanged ? getLatestCart() : Promise.resolve(cart));
      }).then(function (cleanCart) {
        return syncPremiumGiftBox(cleanCart).then(function (premiumChanged) {
          return (premiumChanged ? getLatestCart() : Promise.resolve(cleanCart)).then(function (premiumCart) {
          return syncFreeGift(premiumCart).then(function (freeChanged) {
              return legacyChanged || premiumChanged || freeChanged;
            });
          });
        });
      });
    ...
    .then(function (result) {
      addonPromotionSyncPromise = null;
      return result;
    }, function (error) {
      addonPromotionSyncPromise = null;
      throw error;
...
document.addEventListener('DOMContentLoaded', initAddonPromotion);
document.addEventListener('load', initAddonPromotion);
document.addEventListener('product:added', initAddonPromotion);
document.addEventListener('cart:updated', initAddonPromotion);
document.addEventListener('theme:loading:end', initAddonPromotion);

function removeLegacySleepwearFreeLines(cart) {
  var legacyItems = (cart.items || []).filter(function (item) {
    return item.properties && item.properties[LEGACY_SLEEPWEAR_FREE_MARKER];
  });

  if (!legacyItems.length || pendingAddonRequests.legacySleepwearCleanup) {
    return Promise.resolve(false);
  }

  pendingAddonRequests.legacySleepwearCleanup = true;
  return Promise.all(legacyItems.map(function (item) {
    return fetch('/cart/change.js', {
      ...
      if (!response.ok) {
        throw new Error('Unable to remove legacy sleepwear line');
      }
    });
  })).then(function () {
    delete pendingAddonRequests.legacySleepwearCleanup;
    return true;
  }, function (error) {
    delete pendingAddonRequests.legacySleepwearCleanup;
    throw error;
...
function syncFreeGift(cart) {
  var items = cart.items || [];
  var qualifyingTotal = getCartSubtotalExcludingGiftBoxes(cart);
  var freeGiftItems = items.filter(function (item) {
    return item.variant_id == FREE_GIFT_VARIANT;
  });
  var desiredGiftQuantity = 0;
  var freeGiftItem = freeGiftItems.length ? freeGiftItems[0] : null;
  var currentGiftQuantity = freeGiftItem ? freeGiftItem.quantity : 0;

  FREE_GIFT_TIERS.forEach(function (tier) {
    if (qualifyingTotal >= tier.threshold) {
      desiredGiftQuantity = tier.quantity;
    }
  });

  if (pendingAddonRequests[FREE_GIFT_VARIANT]) {
    return Promise.resolve(false);
  }

  if (!freeGiftItem && desiredGiftQuantity > 0) {
    return updateFreeGift('/cart/add.js', {
      items: [{ id: FREE_GIFT_VARIANT, quantity: desiredGiftQuantity }]
    });
  }

  if (freeGiftItem && currentGiftQuantity !== desiredGiftQuantity) {
    return updateFreeGift('/cart/change.js', {
      id: freeGiftItem.key,
      quantity: desiredGiftQuantity
    });
  }

  if (desiredGiftQuantity === 0 && freeGiftItems.length) {
    return Promise.all(freeGiftItems.map(function (item) {
      return updateFreeGift('/cart/change.js', {
        id: item.key,
        quantity: 0
      });
...
function syncPremiumGiftBox(cart) {
  var items = cart.items || [];
  var baseTotal = getCartSubtotalExcludingGiftBoxes(cart);
  var shouldRemoveGiftBox = baseTotal >= GIFT_BOX_THRESHOLD;
  var premiumGiftItems = items.filter(function (item) {
    return item.variant_id == GIFT_BOX_VARIANT;
  });
  var premiumGiftItem = premiumGiftItems.length ? premiumGiftItems[0] : null;

  if (pendingAddonRequests[GIFT_BOX_VARIANT]) {
    return Promise.resolve(false);
  }

  if (shouldRemoveGiftBox && premiumGiftItem) {
    return updateAddonVariant('/cart/change.js', {
      id: premiumGiftItem.key,
      quantity: 0
    }, GIFT_BOX_VARIANT);
  }
...
function updateFreeGift(url, body) {
  return updateAddonVariant(url, body, FREE_GIFT_VARIANT);
}

function updateAddonVariant(url, body, variantId) {
  pendingAddonRequests[variantId] = true;
  ...
  return fetch(url, {
    ...
    .then(function (response) {
      if (!response.ok) {
        throw new Error('Unable to update addon');
      }
      return true;
    })
    .then(function (result) {
      delete pendingAddonRequests[variantId];
      return result;
    }, function (error) {
      delete pendingAddonRequests[variantId];
      throw error;
...
function refreshPremiumGiftBoxProductState() {
  var premiumGiftBoxSection = document.querySelector('.Product[data-premium-gift-box-variant-id]');

  if (!premiumGiftBoxSection) {
    return Promise.resolve(false);
  }

  var premiumGiftBoxVariantId = parseInt(premiumGiftBoxSection.getAttribute('data-premium-gift-box-variant-id'), 10);
  var selectedVariantInput = premiumGiftBoxSection.querySelector('.ProductForm [name="id"]');
  var selectedVariantId = selectedVariantInput ? parseInt(selectedVariantInput.value, 10) : NaN;
  var buyButtonGroups = premiumGiftBoxSection.querySelectorAll('[data-premium-gift-box-buy-buttons]');
  var notice = premiumGiftBoxSection.querySelector('[data-premium-gift-box-notice]');

  if (selectedVariantId !== premiumGiftBoxVariantId) {
    Array.prototype.forEach.call(buyButtonGroups, function (buttonGroup) {
  ...
  return getLatestCart().then(function (cart) {
    var shouldDisable = getCartSubtotalExcludingGiftBoxes(cart) >= GIFT_BOX_THRESHOLD;

    Array.prototype.forEach.call(buyButtonGroups, function (buttonGroup) {
      buttonGroup.hidden = shouldDisable;
    });

    if (notice) {
      notice.hidden = !shouldDisable;
    }
...
document.addEventListener('change', function (e) {
    ...
    if (!checkbox || checkbox.type !== 'checkbox' || !checkbox.id) {
        return;
    }
    ...
    var variantId = parseInt(checkbox.getAttribute('data-variant-id'), 10);

    if (variantId !== GIFT_BOX_VARIANT) {
        return;
    }

    if (checkbox.checked) {
        addAddonVariant(GIFT_BOX_VARIANT);
    } else {
        removeAddonVariant(GIFT_BOX_VARIANT);
    }
});

document.addEventListener('DOMContentLoaded', refreshPremiumGiftBoxProductState);
document.addEventListener('cart:updated', refreshPremiumGiftBoxProductState);
document.addEventListener('cart:refresh', refreshPremiumGiftBoxProductState);
document.addEventListener('variant:changed', refreshPremiumGiftBoxProductState);

function addAddonVariant(variantId) {
    if (pendingAddonRequests[variantId]) {
        return;
    }
...
function removeAddonVariant(variantId) {
    if (pendingAddonRequests[variantId]) {
        return;
    }
    ...
    fetch('/cart.js')
        ...
        .then(function (cart) {
            ...
            return Promise.all(matchingItems.map(function (item) {
                return fetch('/cart/change.js', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
...
function refreshCartDrawer() {
  var cartUrl = window.routes && window.routes.cartUrl ? window.routes.cartUrl : '/cart';

  return fetch(cartUrl + '?section_id=mini-cart&timestamp=' + Date.now()).then(function (response) {
    return response.text();
  }).then(function (html) {
    var temp = document.createElement('div');
    temp.innerHTML = html;

    var cartContainer = document.querySelector('#sidebar-cart .Cart');

    if (cartContainer && temp.querySelector('.Cart')) {
      cartContainer.replaceWith(temp.querySelector('.Cart'));
    }
